...
# buscar 3 mayusc rodeando a 1 minusc ... ejemplo AAAbCCC ,

import re

def encontrar_letras_minusc_entre_mayusc(cadena):
    # Patrón actualizado para buscar el nuevo formato de letras minúsculas rodeadas de mayúsculas
    ...
    for match in coincidencias:
        # Obtenemos la letra minúscula
        letra_minusc = match.group(1)
        # Obtenemos las mayúsculas antes y después de la letra minúscula
        mayusc_antes = match.group(0)[:3]
        mayusc_despues = match.group(0)[-3:]
...
letras_y_mayusc = encontrar_letras_minusc_entre_mayusc(cadena_ejemplo)
print("Letra minúscula y sus mayúsculas circundantes:")
for letra, mayusc_antes, mayusc_despues in letras_y_mayusc:
    print(f"Letra minúscula: {letra}, Mayúsculas antes: {mayusc_antes}, Mayúsculas después: {mayusc_despues}")





# Crear un diccionario para almacenar las subcadenas y sus ocurrencias
ocurrencias = {}

# Recorrer la cadena de texto
for i in range(len(letra)):
    # Obtener subcadenas de longitud creciente
    for j in range(i+1, len(letra)+1):
        subcadena = letra[i:j]
        
        # Contar la ocurrencia de la subcadena en el diccionario
        if subcadena in ocurrencias:
            ocurrencias[subcadena] += 1
        else:
            ocurrencias[subcadena] = 1

# Filtrar las subcadenas que se repiten
repetidas = {subcadena:ocurrencia for subcadena, ocurrencia in ocurrencias.items() if ocurrencia > 1}

 # Imprimir las subcadenas repetidas
print("Subcadenas repetidas:")
for subcadena in repetidas:
    print(subcadena) 


for letra, mayusc_antes, mayusc_despues in letras_y_mayusc:
    print(f"{mayusc_antes}, {letra}, {mayusc_despues}")

 


# Iterar sobre las letras y mayúsculas circundantes e imprimir solo las letras de forma continua
print("Letras de forma continua:")
for letra, _, _ in letras_y_mayusc:
    print(letra, end='')

# Salto de línea al final para una mejor presentación
print()