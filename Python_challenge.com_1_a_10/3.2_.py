...
# busca  3 mayusc 1 minusc  y 3 mayusc pero tmb encuentra si son mas mayusc las que rodean a la minuscula , no se fija en lo que pasa antes de ejemplo RTYaTYU

# Texto proporcionado
texto = """kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
-->""" 
# Función para encontrar la letra minúscula rodeada por exactamente tres letras mayúsculas en cada lado
def encontrar_letras_minusc_y_mayusc(texto):
    letras_minusc = []
    for i in range(1, len(texto) - 1):
        if texto[i].islower():
            if texto[i - 1].isupper() and texto[i - 2].isupper() and texto[i - 3].isupper():
                if texto[i + 1].isupper() and texto[i + 2].isupper() and texto[i + 3].isupper():
                    letras_minusc.append(texto[i])
    return letras_minusc

# Llamar a la función para encontrar las letras minúsculas
letras_minusc = encontrar_letras_minusc_y_mayusc(texto)

# Imprimir las letras minúsculas encontradas
print("Letras minúsculas encontradas:", letras_minusc)